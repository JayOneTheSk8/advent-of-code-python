def get_increasing_fuel_count(arr, pos):
    fuel_count = 0

    for num in arr:
        difference = abs(num - pos)
...
def get_most_fuel_efficient_position(arr, lowest_num, highest_num):
    ...
    count_before = get_increasing_fuel_count(arr, mid - 1)
    count_current = get_increasing_fuel_count(arr, mid)
    count_after = get_increasing_fuel_count(arr, mid + 1)
